...
- **太长不看**：NPU 的 `hardware_backend/npu/moe/topk.py` 里的 `fused_topk_npu` 方法，少乘了一个 `routed_scaling_factor`，导致每个 MoE 层的路由专家输出只有正确值的约 35%，精度严重下降。修复后 NPU 精度从 **17.5% → 87.88%**，与官方 GPU baseline（87.2%）持平。
...
`routed_scaling_factor` 在 `hunyuan_v3.py` 构造 `TopK` 时传入，**不会立刻做乘法**，而是存进 `self.topk_config`（`TopKConfig`）。每次 MoE forward 调用 `self.topk(...)` 时，才通过 `topk_config=self.topk_config` 传下去。

`TopK` 继承 `MultiPlatformOp`。**对象创建时**就根据平台选好实现（NPU → `forward_npu`，GPU → `forward_cuda`）；运行时 `self.topk(...)` 实际走的是 `TopK.forward()` → `self._forward_method(...)`。

追踪完整调用链：

```
HYV3MoEFused._forward_single_stream()
  router_logits = self.gate(hidden_states)
  topk_output = self.topk(hidden_states, router_logits)
      │
      ▼
TopK.forward()                         # MultiPlatformOp：按平台分发
  → self._forward_method(...)          # NPU: forward_npu / GPU: forward_cuda
      │
      ├─ NPU 路径
      │   TopK.forward_npu()           # layers/moe/topk.py:550
      │       → fused_topk_npu(..., topk_config=self.topk_config)
      │           → npu_moe_gating_top_k(
      │                 routed_scaling_factor=(
      │                     1 if renormalize   # ← 故意传 1，不在 op 内部缩放
      │                     else topk_config.routed_scaling_factor
      │                 )
      │             )
      │           # op 返回 renorm 后的权重，sum ≈ 1
      │           # ← 修复前：缺少 topk_weights *= routed_scaling_factor ✗
      │           # ← 修复后：补上后乘，与 GPU 对齐 ✓
      │
      └─ GPU 路径
          TopK.forward_cuda()          # layers/moe/topk.py:468
              → select_experts(..., topk_config=self.topk_config)
                  → biased_grouped_topk(...)   # HY3 走这条（use_grouped_topk + correction_bias）
                      → renormalize            # sum 变为 1
                      → topk_weights *= routed_scaling_factor  # ← 乘以 2.826 ✓
```

**GPU 路径**（`biased_grouped_topk_impl`，`layers/moe/topk.py:1133-1141`）：

```python
if renormalize:
    topk_weights_sum = topk_weights.sum(dim=-1, keepdim=True)
    topk_weights = topk_weights / topk_weights_sum   # renorm，sum 变为 1
    if apply_routed_scaling_factor_on_output:
        topk_weights *= routed_scaling_factor        # ← 再乘回缩放系数
```

**NPU 路径**（`fused_topk_npu`，`hardware_backend/npu/moe/topk.py`）：
...
| 指标 | 修复前 NPU | 修复后 NPU | 官方 GPU baseline |
|------|-----------|-----------|-----------------|
| GPQA Diamond 精度 | 17.5%（40题）| **87.88%**（198题）| 87.2% |
| 死循环比例 | ~35% 题目 | 0 | 0 |

NPU 修复后精度（87.88%）已超过官方 GPU baseline（87.2%），问题彻底解决。
